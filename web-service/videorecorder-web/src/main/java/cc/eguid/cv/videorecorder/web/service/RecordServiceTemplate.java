...
public abstract class RecordServiceTemplate implements RecordService{
	...
	public synchronized void init() {
		try {
			if(manager==null) {
				RecordInfoStorage cache=initCache();//持久化实现
				manager = new DefaultTasksManager(maxSize).setPlayUrl(playurl).setRecordDir(dir).setHistoryStorage(cache);
				status = 1;
			}
		} catch (Exception e) {
			status = -1;
	...
	public RecordInfo get(Integer id) {
		RecordInfo info=  manager.getRcorderInfo(id);
